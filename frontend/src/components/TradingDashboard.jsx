...
const TradingDashboard = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      console.log('Submitting hypothesis:', requestData);
      ...
      setError(err.message || 'Failed to process hypothesis. Please try again.');
  ...
  const getStatusColor = (status) => {
    ...
    if (!statusValue) {
      console.warn('Status is undefined or null, using default color');
                  ...
                  {sidebarCollapsed ? (
                    ...
                    <>
                      <h3 className="font-semibold text-gray-800 text-sm mb-2 line-clamp-2">
                        {formatHypothesisTitle(hyp.title || hyp.processed_hypothesis || 'Untitled Hypothesis')}
                      </h3>
                      <div className="flex items-center space-x-3 text-xs">
                        <span className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(hyp.status || hyp.status?.toLowerCase() || 'pending')}`}>
                          {hyp.status || 'pending'}
                        </span>
                        {hyp.confidence_score !== undefined && (
                          <span className="text-gray-500">{Math.round(hyp.confidence_score * 100)}%</span>
                        )}
                      </div>
                      {hyp.contradictions !== undefined && hyp.confirmations !== undefined && (
                        <div className="flex justify-between text-xs text-gray-500 mt-2">
                          <span>{hyp.contradictions || 0}✗</span>
                          <span>{hyp.confirmations || 0}✓</span>
                        </div>
                      )}
                    </>
                  )}
